...
object Formatting {
  ...
  def suberText[_: P]: P[FString] = P(!suberStop ~ char).rep(0).map(FString)

  def formattedCurlyBracketText[_: P]: P[FormattedString] = (!P("}") ~ formattedToken).rep(0).map(FString)
  ...
  def formattedCode[_: P]: P[FormattedString] = (!P("`") ~ char).rep(0).map(FString)
  ...
  def formattedToken[_: P]: P[FormattedString] = P(bold | italic | code | superscript | subscript | !endLines ~ char)
